...
Shader "Shader Forge/Water" {
    Properties {
        ...
        _ColorPower ("ColorPower", Float ) = 0.4
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 node_2693 = ((i.uv0*5.0)+node_5115.g*float2(0.02,0.02));
                ...
                float2 node_4939 = ((i.uv0*100.0)+node_5115.g*float2(-0.03,-0.03));
...
////// Specular:
                ...
                float3 diffuseColor = (lerp(_BoardColor_A.rgb,_BoardColor_B.rgb,node_5538)*pow(1.0-max(0,dot(node_9406, viewDirection)),_Fresnel)); // Need this for specular when using metallic
...
////// Emissive:
                float3 emissive = (_ColorPower*_BoardColor_A.rgb);
/// Final Color:
                float3 finalColor = diffuse + specular + emissive;
                fixed4 finalRGBA = fixed4(lerp(sceneColor.rgb, finalColor,(saturate((node_5538.r*0.5))*i.vertexColor.a)),1);
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3x3 tangentTransform = float3x3( i.tangentDir, i.bitangentDir, i.normalDir);
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
                float4 node_2694 = _Time;
                float2 node_2693 = ((i.uv0*5.0)+node_2694.g*float2(0.02,0.02));
                float3 _node_8693_var = UnpackNormal(tex2D(_node_8693,TRANSFORM_TEX(node_2693, _node_8693)));
                float3 node_9406 = lerp(float3(0,0,3),_node_8693_var.rgb,_Reflection);
                float2 node_4939 = ((i.uv0*100.0)+node_2694.g*float2(-0.03,-0.03));
                float3 _NormalDetail_var = UnpackNormal(tex2D(_NormalDetail,TRANSFORM_TEX(node_4939, _NormalDetail)));
                float3 node_9518_nrm_base = node_9406 + float3(0,0,1);
                float3 node_9518_nrm_detail = _NormalDetail_var.rgb * float3(-1,-1,1);
                float3 node_9518_nrm_combined = node_9518_nrm_base*dot(node_9518_nrm_base, node_9518_nrm_detail)/node_9518_nrm_base.z - node_9518_nrm_detail;
...
/// Final Color:
                ...
                fixed4 finalRGBA = fixed4(finalColor * (saturate((node_5538.r*0.5))*i.vertexColor.a),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                o.Emission = (_ColorPower*_BoardColor_A.rgb);
                
                float node_4964 = saturate((saturate((sceneZ-partZ)/_Depth)/0.5*dot(viewDirection,i.normalDir)+0.5));
                float node_6011 = 0.0;
                float node_5538 = saturate((_FoamPower*saturate((node_6011 + ( (node_4964 - _FoamMin) * (1.0 - node_6011) ) / (_FoamMax - _FoamMin)))));
                float4 node_1234 = _Time;
                float2 node_2693 = ((i.uv0*5.0)+node_1234.g*float2(0.02,0.02));
                float3 _node_8693_var = UnpackNormal(tex2D(_node_8693,TRANSFORM_TEX(node_2693, _node_8693)));
                float3 node_9406 = lerp(float3(0,0,3),_node_8693_var.rgb,_Reflection);
                float3 diffColor = (lerp(_BoardColor_A.rgb,_BoardColor_B.rgb,node_5538)*pow(1.0-max(0,dot(node_9406, viewDirection)),_Fresnel));
                float specularMonochrome;
                float3 specColor;
